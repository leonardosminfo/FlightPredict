...
class_mlp_nnet <- function(data, test, clabel, neurons=3, decay=0.01, iterations=5000, MaxNWts = 1000)
{
  ...
  model = nnet(data_predictors, data_predictand_clabel, size=neurons, decay=decay, maxit=iterations, MaxNWts = MaxNWts)
...
class_knn <- function(data, test, clabel, k = 10, use.all = TRUE)
{
  predictors_name  = setdiff(colnames(data), clabel)
  ...
  predictions_train = knn(data_predictors, data_predictors, data_clabel, k=k, prob=TRUE, use.all = use.all)
  predictions = knn(data_predictors, test_predictors, data_clabel, k=k, prob=TRUE, use.all = use.all)
