...
categoricDataSmote <- function(df) {
	idx_categoric <- grep("aeroporto|sigla|time|conditions|mes|diasemana|wdirection", colnames(df))
	
	for (idx in idx_categoric) {
		df[[idx]] <- ifelse(df[[idx]] >= 0.5, 1, 0)
	}
...
dt.pca <- function(data, clabel, transf = NULL){
  ...
  if (is.null(transf)) {
    ...
    pca_res = prcomp(predictors, center=TRUE, scale.=FALSE)
